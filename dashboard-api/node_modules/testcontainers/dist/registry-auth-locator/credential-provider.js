...
class CredentialProvider {
    getAuthConfig(registry, dockerConfig) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const obfuscatedAuthConfig = JSON.stringify(Object.assign(Object.assign({}, authConfig), { password: "*".repeat(10) }));
            logger_1.log.debug(`Docker credential provider found auth config for ${registry}: ${obfuscatedAuthConfig}`);
            return authConfig;
        });
    }
    listCredentials(providerName) {
        return new Promise((resolve, reject) => {
            child_process_1.exec(`${providerName} list`, (err, stdout) => {
                if (err) {
                    logger_1.log.error("An error occurred listing credentials");
                    return reject(err);
                }
                try {
    ...
    runCredentialProvider(registry, providerName) {
        return new Promise((resolve) => {
            const sink = child_process_1.spawn(providerName, ["get"]);
            const chunks = [];
            sink.stdout.on("data", (chunk) => chunks.push(chunk));
            sink.on("close", (code) => {
                if (code === 0) {
                    logger_1.log.debug(`Docker credential provider exited with code: ${code}`);
                }
                else {
                    logger_1.log.warn(`Docker credential provider exited with code: ${code}`);
                }
