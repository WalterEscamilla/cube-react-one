...
class CubeStoreDriver extends query_orchestrator_1.BaseDriver {
    ...
    getPrefixTablesQuery(schemaName, tablePrefixes) {
        const prefixWhere = tablePrefixes.map(p => 'table_name LIKE CONCAT(?, \'%\')').join(' OR ');
        return this.query(`SELECT table_name FROM information_schema.tables WHERE table_schema = ${this.param(0)} AND (${prefixWhere})`, [schemaName].concat(tablePrefixes));
    }
