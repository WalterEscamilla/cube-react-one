...
class PreAggregationPartitionRangeLoader {
    ...
    async partitionRanges() {
        ...
        let dateRange = PreAggregationPartitionRangeLoader.intersectDateRanges([startDate, endDate], this.preAggregation.matchedTimeDimensionDateRange);
        if (!dateRange) {
        ...
        let timeSeriesRanges = PreAggregationPartitionRangeLoader.timeSeries(this.preAggregation.partitionGranularity, dateRange);
        ...
        const [preciseStartDate, preciseEndDate] = await Promise.all(preAggregationStartEndQueries.map(async (rangeQuery, i) => PreAggregationPartitionRangeLoader.extractDate(await this.loadRangeQuery(rangeQuery, i === 0 ? wholeSeriesRanges[0] : wholeSeriesRanges[wholeSeriesRanges.length - 1]))));
        if (preciseStartDate !== startDate || preciseEndDate !== endDate) {
            dateRange = PreAggregationPartitionRangeLoader.intersectDateRanges([preciseStartDate, preciseEndDate], this.preAggregation.matchedTimeDimensionDateRange);
            if (!dateRange) {
                // If there's no date range intersection between query data range and pre-aggregation build range
                // use last partition so outer query can receive expected table structure.
                dateRange = [preciseEndDate, preciseEndDate];
            }
            timeSeriesRanges = PreAggregationPartitionRangeLoader.timeSeries(this.preAggregation.partitionGranularity, dateRange);
        }
