...
class ApiGateway {
        ...
        try {
            ...
            const mergePartitionsAndVersionEntries = () => ({ preAggregation, partitions, ...props }) => ({
                ...
                partitions: partitions.map(partition => {
                    var _a, _b;
                    partition.versionEntries = (versionEntriesResult === null || versionEntriesResult === void 0 ? void 0 : versionEntriesResult.versionEntriesByTableName[(_a = partition.sql) === null || _a === void 0 ? void 0 : _a.tableName]) || [];
                    partition.structureVersion = versionEntriesResult === null || versionEntriesResult === void 0 ? void 0 : versionEntriesResult.structureVersionsByTableName[(_b = partition.sql) === null || _b === void 0 ? void 0 : _b.tableName];
                    return partition;
                }),
            });
            res({
    ...
    async getPreAggregationPreview({ query, context, res }) {
        ...
        try {
            ...
            const compilerApi = this.getCompilerApi(context);
            const preAggregationPartitions = await this.refreshScheduler()
                .preAggregationPartitions(context, compilerApi, {
                timezones: [timezone],
                preAggregations: [{ id: preAggregationId }]
            });
            const { partitions } = (preAggregationPartitions && preAggregationPartitions[0] || {});
            const preAggregationPartition = partitions && partitions.find(p => { var _a; return ((_a = p.sql) === null || _a === void 0 ? void 0 : _a.tableName) === versionEntry.table_name; });
            res({
                preview: preAggregationPartition && await orchestratorApi.getPreAggregationPreview(context, preAggregationPartition, query.versionEntry)
            });
        }
        catch (e) {
            this.handleError({
    ...
    async buildPreAggregations({ query, context, res }) {
        ...
        try {
            ...
            const result = await this.refreshScheduler()
                .buildPreAggregations(context, this.getCompilerApi(context), query);
            res({ result });
        }
        catch (e) {
            this.handleError({
    ...
    async getNormalizedQueries(query, context) {
        ...
        const normalizedQueries = await Promise.all(queries.map((currentQuery) => this.queryRewrite(query_1.normalizeQuery(currentQuery), context)));
