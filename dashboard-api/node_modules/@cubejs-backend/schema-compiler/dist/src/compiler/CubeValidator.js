...
const BasePreAggregationWithoutPartitionGranularity = {
    refreshKey: Joi.alternatives().try(Joi.object().keys({
        sql: Joi.func().required(),
...
const cubeSchema = Joi.object().keys({
    ...
    measures: Joi.object().pattern(identifierRegex, Joi.alternatives().try(Joi.object().keys(Object.assign({}, BaseMeasure, {
        sql: Joi.func(),
        type: Joi.any().valid('count').required()
    })), Joi.object().keys(Object.assign({}, BaseMeasure, {
        sql: Joi.func().required(),
        type: Joi.any().valid('number', 'sum', 'avg', 'min', 'max', 'countDistinct', 'runningTotal', 'countDistinctApprox').required()
    })))),
    dimensions: Joi.object().pattern(identifierRegex, Joi.alternatives().try(Joi.object().keys(Object.assign({}, BaseDimensionWithoutSubQuery, {
    ...
    })), Joi.object().keys(Object.assign({}, BaseDimensionWithoutSubQuery, {
    ...
    })), Joi.object().keys(Object.assign({}, BaseDimension, {
        sql: Joi.func().required()
    })))),
    ...
    preAggregations: Joi.object().pattern(identifierRegex, Joi.alternatives().try(Joi.object().keys(Object.assign({}, BasePreAggregation, {
    ...
    Joi.object().keys(Object.assign({}, BasePreAggregation, {
        ...
        segmentReferences: Joi.func(),
        timeDimensionReference: Joi.func().required(),
        granularity: Joi.any().valid('second', 'minute', 'hour', 'day', 'week', 'month', 'year').required(),
        rollupReferences: Joi.func().required(),
    })), 
