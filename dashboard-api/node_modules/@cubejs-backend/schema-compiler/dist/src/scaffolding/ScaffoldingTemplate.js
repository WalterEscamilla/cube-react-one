...
const ValueWithComments_1 = require("./ValueWithComments");
...
class ScaffoldingTemplate {
    constructor(dbSchema, driver) {
        ...
        this.scaffoldingSchema = new ScaffoldingSchema_1.ScaffoldingSchema(dbSchema);
        this.driver = driver;
    }
    escapeName(name) {
    ...
    resolveTableName(tableName) {
        ...
        if (tableParts.length === 2) {
            ...
            if (!schema) {
                throw new UserError_1.UserError(`Can't find any table with '${tableName}' name`);
            }
            ...
            throw new UserError_1.UserError('Table names should be in <table> or <schema>.<table> format');
        }
    }
    schemaDescriptorForTable(tableSchema, schemaContext = {}) {
        return {
            cube: tableSchema.cube,
            sql: `SELECT * FROM ${this.escapeName(tableSchema.schema)}.${this.escapeName(tableSchema.table)}`,
