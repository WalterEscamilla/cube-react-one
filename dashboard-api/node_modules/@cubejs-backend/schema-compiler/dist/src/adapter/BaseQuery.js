...
class BaseQuery {
    ...
    get dataSource() {
        ...
        if (dataSources.length > 1 && !this.externalPreAggregationQuery()) {
            throw new UserError_1.UserError(`To join across data sources use rollupJoin with external pre-aggregations. Found data sources: ${dataSources.join(', ')}`);
        }
    ...
    initUngrouped() {
        ...
        if (this.ungrouped) {
            ...
            if (this.measures.length) {
                throw new UserError_1.UserError('Measures aren\'t allowed in ungrouped query');
            }
    ...
    newGroupFilter(filter) {
        return new BaseGroupFilter_1.BaseGroupFilter(this, filter);
    }
    newTimeDimension(timeDimension) {
        return new BaseTimeDimension_1.BaseTimeDimension(this, timeDimension);
    }
    newParamAllocator() {
        return new ParamAllocator_1.ParamAllocator();
    }
    newPreAggregations() {
    ...
    buildParamAnnotatedSql() {
        if (!this.options.preAggregationQuery && !this.ungrouped) {
            ...
            if (preAggregationForQuery) {
                ...
                ] : []).concat(ramda_1.default.map(([multiplied, measure]) => this.withCubeAliasPrefix(`${this.aliasName(measure.measure.replace('.', '_'))}_cumulative`, () => this.overTimeSeriesQuery((measures, filters) => this.preAggregations.rollupPreAggregation(preAggregationForQuery, measures, false, filters), measure, true)))(cumulativeMeasures));
    ...
    fullKeyQueryAggregate() {
        ...
        let toJoin;
        if (this.options.preAggregationQuery) {
            const allRegular = regularMeasures.concat(cumulativeMeasures.map(([multiplied, measure]) => (multiplied ? null : measure)).filter(m => !!m));
            const allMultiplied = multipliedMeasures.concat(cumulativeMeasures.map(([multiplied, measure]) => (multiplied ? measure : null)).filter(m => !!m));
            ...
            toJoin =
                (regularMeasures.length ? [
                    ...
                    .concat(ramda_1.default.pipe(ramda_1.default.groupBy(m => m.cube().name), ramda_1.default.toPairs, ramda_1.default.map(([keyCubeName, measures]) => this.withCubeAliasPrefix(`${keyCubeName}_key`, () => this.aggregateSubQuery(keyCubeName, measures))))(multipliedMeasures)).concat(ramda_1.default.map(([multiplied, measure]) => this.withCubeAliasPrefix(`${this.aliasName(measure.measure.replace('.', '_'))}_cumulative`, () => this.overTimeSeriesQuery(multiplied ?
        ...
        if (regularMeasures.length &&
            ...
            if (ramda_1.default.equals(regularMeasuresCubes, multipliedMeasuresCubes)) {
                toJoin = ramda_1.default.pipe(ramda_1.default.groupBy(m => m.cube().name), ramda_1.default.toPairs, ramda_1.default.map(([keyCubeName, measures]) => this.withCubeAliasPrefix(`${keyCubeName}_key`, () => this.aggregateSubQuery(keyCubeName, measures))))(regularMeasures.concat(multipliedMeasures));
            }
        }
        return this.joinFullKeyQueryAggregate(multipliedMeasures, regularMeasures, cumulativeMeasures, toJoin);
    }
    ...
    granularityParentHierarchy(granularity) {
        if (!this.granularityParentHierarchyCache[granularity]) {
            this.granularityParentHierarchyCache[granularity] = [granularity].concat(this.granularityParent(granularity) ? this.granularityParentHierarchy(this.granularityParent(granularity)) : []);
        }
    ...
    overTimeSeriesQuery(baseQueryFn, cumulativeMeasure, fromRollup) {
        ...
        const dateFromStartToEndConditionSql = (isFromStartToEnd) => dateJoinCondition.map(
    ...
    collectRootMeasureToHieararchy() {
        ...
        return ramda_1.default.fromPairs(this.measures.concat(notAddedMeasureFilters).map(m => {
            const collectedMeasures = this.collectFrom([m], this.collectMultipliedMeasures.bind(this), 'collectMultipliedMeasures', this.queryCache);
    ...
    checkShouldBuildJoinForMeasureSelect(measures, keyCubeName) {
        return measures.map(measure => {
            ...
            if (ramda_1.default.any(cubeName => keyCubeName !== cubeName, cubeNames)) {
    ...
    keysQuery(primaryKeyDimension, filters) {
        const inlineWhereConditions = [];
        const query = this.rewriteInlineWhere(() => this.joinQuery(this.join, this.collectFrom(this.keyDimensions(primaryKeyDimension), this.collectSubQueryDimensionsFor.bind(this), 'collectSubQueryDimensionsFor')), inlineWhereConditions);
        return `SELECT DISTINCT ${this.keysSelect(primaryKeyDimension)} FROM ${query} ${this.baseWhere(filters.concat(inlineWhereConditions))}`;
    }
    keysSelect(primaryKeyDimension) {
        return ramda_1.default.flatten(this.keyDimensions(primaryKeyDimension)
            .map(s => s.selectColumns())).filter(s => !!s).join(', ');
    }
    keyDimensions(primaryKeyDimension) {
        return this.dimensionsForSelect()
            .concat(ramda_1.default.none(d => d.dimension === primaryKeyDimension.dimension, this.dimensionsForSelect()) ? [primaryKeyDimension] : []);
    }
    cubeSql(cube) {
    ...
    collectFrom(membersToCollectFrom, fn, methodName, cache) {
        return ramda_1.default.pipe(ramda_1.default.map(f => f.getMembers()), ramda_1.default.flatten, ramda_1.default.map(s => ((cache || this.compilerCache).cache(['collectFrom', methodName].concat(s.path() ? [s.path().join('.')] : [s.cube().name, s.expressionName || s.definition().sql]), () => fn(() => this.traverseSymbol(s))))), ramda_1.default.unnest, ramda_1.default.uniq, ramda_1.default.filter(ramda_1.default.identity))(membersToCollectFrom);
    }
    ...
    getFieldIndex(id) {
        ...
        let index;
        index = this.dimensionsForSelect().findIndex(d => equalIgnoreCase(d.dimension, id));
        if (index > -1) {
            return index + 1;
        }
    ...
    evaluateSymbolSql(cubeName, name, symbol) {
        ...
        if (this.cubeEvaluator.isMeasure([cubeName, name])) {
            ...
            const result = this.renderSqlMeasure(name, this.applyMeasureFilters(this.autoPrefixWithCubeName(cubeName, symbol.sql && this.evaluateSql(cubeName, symbol.sql) ||
                this.cubeEvaluator.primaryKeys[cubeName] && this.primaryKeySql(this.cubeEvaluator.primaryKeys[cubeName], cubeName) || '*'), symbol, cubeName), symbol, cubeName, parentMeasure);
            if (this.safeEvaluateSymbolContext().compositeCubeMeasures ||
                this.safeEvaluateSymbolContext().leafMeasures) {
    ...
    primaryKeyName(cubeName) {
        ...
        if (!primaryKey) {
            throw new UserError_1.UserError(`Primary key is required for '${cubeName}`);
        }
    ...
    collectMultipliedMeasures(fn) {
        ...
        this.evaluateSymbolSqlWithContext(fn, { compositeCubeMeasures: foundCompositeCubeMeasures });
        const renderContext = {
    ...
    inDbTimeZone(date) {
        return shared_1.inDbTimeZone(this.timezone, this.timestampFormat(), date);
    }
    /**
     * @return {string}
     */
    ...
    newSubQuery(options) {
        ...
        return new QueryClass(this.compilers, Object.assign({
    ...
    parseInterval(interval) {
        ...
        if (duration < 1) {
            throw new UserError_1.UserError(`Duration should be positive: ${interval}`);
        }
            ...
            if (preAggregation.refreshKey) {
                ...
                if (preAggregation.refreshKey.incremental) {
                    if (!preAggregation.partitionGranularity) {
                        throw new UserError_1.UserError('Incremental refresh key can only be used for partitioned pre-aggregations');
                    }
            ...
            if (preAggregation.partitionGranularity &&
                ...
                return preAggregationQueryForSql.evaluateSymbolSqlWithContext(() => preAggregationQueryForSql.cacheKeyQueries((refreshKeyCube, [refreshKeySQL, refreshKeyQueryOptions, refreshKeyQuery]) => {
                    if (cubeFromPath.refreshKey && cubeFromPath.refreshKey.immutable) {
                         ...
                         */
                        return [
                            this.refreshKeySelect(this.incrementalRefreshKey(preAggregationQueryForSql, `(${refreshKeySQL})`, {
                                refreshKeyQuery
                            })),
                            {
