...
moment_timezone_1.default.fn.zoneName = () => {
    const abbr = this.zoneAbbr();
    return abbrs[abbr] || abbr;
};
class MssqlParamAllocator extends ParamAllocator_1.ParamAllocator {
...
const GRANULARITY_TO_INTERVAL = {
    ...
    second: (date) => `CAST(FORMAT(${date}, 'yyyy-MM-ddTHH:mm:ss.000') AS DATETIME2)`,
    month: (date) => `dateadd(month, DATEDIFF(month, 0, ${date}), 0)`,
    year: (date) => `dateadd(year, DATEDIFF(year, 0, ${date}), 0)`,
};
class MssqlFilter extends BaseFilter_1.BaseFilter {
...
class MssqlQuery extends BaseQuery_1.BaseQuery {
    newFilter(filter) {
        return new MssqlFilter(this, filter);
    }
    convertTz(field) {
        return `TODATETIMEOFFSET(${field}, '${moment_timezone_1.default().tz(this.timezone).format('Z')}')`;
    }
    timeStampCast(value) {
    ...
    overTimeSeriesSelect(cumulativeMeasures, dateSeriesSql, baseQuery, dateJoinConditionSql, baseQueryAlias) {
        const forGroupBy = this.timeDimensions.map((t) => `${t.dateSeriesAliasName()}.${this.escapeColumnName('date_from')}`);
...
//# sourceMappingURL=MssqlQuery.js.map