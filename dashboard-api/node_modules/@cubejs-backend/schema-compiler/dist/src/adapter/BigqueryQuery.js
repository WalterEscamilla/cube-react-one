...
class BigqueryFilter extends BaseFilter_1.BaseFilter {
...
class BigqueryQuery extends BaseQuery_1.BaseQuery {
    convertTz(field) {
    ...
    overTimeSeriesSelect(cumulativeMeasures, dateSeriesSql, baseQuery, dateJoinConditionSql, baseQueryAlias) {
        ...
        const aliasesForSelect = this.timeDimensions.map(d => d.dateSeriesSelectColumn(outerSeriesAlias)).concat(this.dimensions.concat(cumulativeMeasures).map(s => s.aliasName())).filter(c => !!c).join(', ');
...
exports.BigqueryQuery = BigqueryQuery;
//# sourceMappingURL=BigqueryQuery.js.map