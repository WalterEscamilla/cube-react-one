...
class PreAggregations {
    ...
    preAggregationDescriptionFor(cube, foundPreAggregation) {
        ...
        const matchedTimeDimension = preAggregation.partitionGranularity &&
            !this.hasCumulativeMeasures &&
            this.query.timeDimensions.find(td => td.dimension === foundPreAggregation.references.timeDimensions[0].dimension && td.dateRange);
        const filters = preAggregation.partitionGranularity && this.query.filters.filter(td => td.dimension === foundPreAggregation.references.timeDimensions[0].dimension &&
            td.isDateOperator() &&
        ...
        return {
            ...
            preAggregationStartEndQueries: preAggregation.partitionGranularity &&
    ...
    static canUsePreAggregationForTransformedQueryFn(transformedQuery, refs) {
        ...
        const windowGranularityMatches = (references) => {
            ...
            return expandGranularity(transformedQuery.windowGranularity).map(windowGranularity => ramda_1.default.all(td => td[1] === windowGranularity, sortedTimeDimensions)).filter(x => !!x).length > 0;
        };
        const canUsePreAggregationNotAdditive = (references) => transformedQuery.hasNoTimeDimensionsWithoutGranularity &&
            ...
            ramda_1.default.equals(transformedQuery.sortedTimeDimensions, references.sortedTimeDimensions || sortTimeDimensions(references.timeDimensions)) && !transformedQuery.hasCumulativeMeasures;
    ...
    rollupPreAggregation(preAggregationForQuery, measures, isFullSimpleQuery, filters) {
        ...
        const from = this.query.joinSql(toJoin.map(j => ({
            ...j,
            sql: this.query.preAggregationTableName(j.preAggregation.cube, 
            // @todo Dont use sqlAlias directly, we needed to move it in preAggregationTableName
        ...
        const renderedReference = ramda_1.default.pipe(ramda_1.default.map(path => {
            ...
            return [
                path,
                this.query.aggregateOnGroupedColumn(measure.measureDefinition(), measure.aliasName(), !this.query.safeEvaluateSymbolContext().overTimeSeriesAggregate) || `sum(${measure.aliasName()})`
            ];
